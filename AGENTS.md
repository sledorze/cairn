...
3. Run this repo's configured stamp command (`stampCommand` in `.cairnrc.json`) to
   (re)write the sidecar hashes under `.cairn/` bottom-up: `pnpm format && npx cairn
check --summaries-only --stamp` — format first, since stamping before a later
   reformat hashes content the format step is about to change.
