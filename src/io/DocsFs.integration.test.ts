...
describe('DocsFsLive()', () => {
  ...
  // Adversarial finding, security-relevant (issue #28's PR, 6th review
  // pass): round 5's containment check applies only to a symlink
  // DISCOVERED while recursing — a caller-supplied ROOT itself (e.g. a
  // literal `docs` directory a malicious PR replaces with a symlink to an
  // absolute path on the CI runner, via git's own symlink mode 120000)
  // was never checked at all. Every checker calls `listFiles(roots,
  // ignore)` with lexical roots derived from `cwd` — an optional `base`
  // (the checkout root every checker already has) lets `listFiles` verify
  // each root's own real path stays inside it, same fix shape as every
  // nested-symlink case, just one level up. `undefined` (the default)
  // preserves existing behavior byte for byte — this is opt-in per
  // caller, not a forced breaking change to every existing `listFiles` call.
  describe("a caller-supplied root that is itself a symlink escaping `base` (issue #28's PR, round 6)", () => {
    const isRoot = typeof process.getuid === 'function' && process.getuid() === 0
    const supportsSymlinks = process.platform !== 'win32' && !isRoot

    it.skipIf(!supportsSymlinks)(
      'a root symlinked to a real target OUTSIDE `base` is excluded entirely, not scanned',
      async () => {
        const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'docsfs-root-escape-project-'))
        const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'docsfs-root-escape-outside-'))
        try {
          fs.writeFileSync(path.join(outsideDir, 'secret.md'), '# not part of the repo')
          ...
          const files = await run(
            Effect.gen(function* () {
              const dfs = yield* DocsFs
              return yield* dfs.listFiles([docsRoot], [], projectRoot)
            }),
          )
    ...
    it.skipIf(!supportsSymlinks)(
      'a root symlinked to a real target INSIDE `base` is still scanned normally',
      async () => {
        const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'docsfs-root-inside-project-'))
        try {
          fs.mkdirSync(path.join(projectRoot, 'real-docs'))
          fs.writeFileSync(path.join(projectRoot, 'real-docs', 'a.md'), '# a')
          const docsRoot = path.join(projectRoot, 'docs')
          fs.symlinkSync(path.join(projectRoot, 'real-docs'), docsRoot, 'dir')
          const files = await run(
            Effect.gen(function* () {
              const dfs = yield* DocsFs
              return yield* dfs.listFiles([docsRoot], [], projectRoot)
          ...
          // Path is joined from the WALKED (lexical `docs`) location, not the
          // resolved target — same convention every other entry in this file
          // already uses (a nested symlink's OWN lexical path is what's
          // returned/reported too).
          expect(files).toEqual([toPosix(path.join(docsRoot, 'a.md'))])
        } finally {
          fs.rmSync(projectRoot, { force: true, recursive: true })
        }
      },
    ...
    // Matches this file's own "permission-denied ROOT" precedent (above):
    // a directory the CALLER explicitly named must fail LOUDLY if
    // something's genuinely wrong with it — silently reading "0 files" as
    // success is the worse failure mode issue #63 already fixed once. A
    // broken/looping symlink at the root itself is exactly that class of
    // problem, so `realPath`'s failure here propagates via `Effect.orDie`,
    // unlike every nested-entry check elsewhere in this function (which
    // gets the lenient silent-exclude treatment).
    it.skipIf(!supportsSymlinks)(
      'a root that is a BROKEN symlink fails loudly when `base` is supplied — never silently reports zero files',
      async () => {
        const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'docsfs-root-broken-symlink-'))
        try {
          const docsRoot = path.join(projectRoot, 'docs')
          // A circular symlink — `fs.exists` (which also follows links)
          // still reports it present (ELOOP resolves differently from
          // ENOENT on most platforms' `exists`), so this reaches the
          // `realPath` call and fails there, not at the earlier `exists` gate.
          fs.symlinkSync(docsRoot, docsRoot)
          await expect(
            run(
              Effect.gen(function* () {
                const dfs = yield* DocsFs
    ...
    it('omitting `base` entirely preserves existing behavior byte for byte — the containment check is opt-in, not forced', async () => {
      const p = fs.mkdtempSync(path.join(os.tmpdir(), 'docsfs-root-no-base-'))
      try {
        fs.writeFileSync(path.join(p, 'a.md'), '# a')
        const files = await run(
          Effect.gen(function* () {
            const dfs = yield* DocsFs
            return yield* dfs.listFiles([p])
          }),
        )
        expect(files).toEqual([toPosix(path.join(p, 'a.md'))])
      } finally {
